import "server-only";
import { getPricingScenarios, getScenarioSettings, getSavedCalculations, getProductsList } from "@/lib/pricing/actions";
...
export default async function PricingProfitabilityPage() {
  // 병렬 데이터 페칭
  const [scenarios, settings, savedCalculations, products] = await Promise.all([
    getPricingScenarios(),
    getScenarioSettings(),
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            공급가 기준 미국 시장(B2B 및 Amazon B2C) 판매 수익성을 시뮬레이션하고 시나리오 기본값을 관리합니다.
          </p>
        </div>
