...
export default async function PortalPurchaseOrdersPage() {
  ...
  // Status mapping and badge helper for canonical overall lifecycle status
  const getOverallStatusBadge = (overallStatus: string) => {
    const label = OVERALL_STATUS_LABELS[overallStatus] || overallStatus || "초안 (Draft)";
    const colorClasses =
  ...
  const getConfirmationBadge = (status: string) => {
    switch (status) {
      ...
      case "REJECTED":
        return <span className="inline-flex items-center rounded-md bg-rose-50 px-2 py-1 text-xs font-medium text-rose-700 ring-1 ring-inset ring-rose-600/20">거절됨</span>;
      default:
